phd_ranks <- phd_field %>% 
  mutate(field = gsub(" psychology", "", field)) %>% 
...
down_field <- c("Cognitive", "Neuropsychology", "Family", "general")
up_field <- c("School", "Cognitive neuroscience", "Marriage", "Health", "other")
...
ggplot(phd_ranks, aes(x = year, y = ranking, group = field, color = swing)) + 
  ...
  labs(x = "Year", y = "Popularity rank", title = "Psychology PhDs awarded by field", subtitle = "With massive increases in marriage psychology, cognitive neuroscience, and school psychology, and drops in more \ngeneralized fields like cognitive psyhology and family counseling...the field seems to be getting more specialized.")
